...
`backend/app/routers/classmates.py` → `GET /api/classmates`. For a signed-in user, returns other students who share at least one course with them — across all enrollment kinds (past / current / upcoming) on BOTH sides. Real cross-table aggregation across `users` × `enrollments` (self-join to find course overlap). The query is symmetric: anyone the viewer has ever overlapped with on any course counts, regardless of whether either party has the course tagged past, current, or upcoming. Each shared course in the response carries the OTHER user's kind so the frontend can color-code (`SharedCourseOut`). If a classmate is enrolled in the same course across multiple terms (retake), they appear once with the highest-priority kind (current > past > upcoming). The requesting user is excluded; classmates are deduplicated even when they share multiple courses.
...
**Real-time-ish chat via polling** (shipped). `<ConversationThread>` polls `GET /api/conversations/:id/messages` every 4s while open and merges new IDs into the existing list, so user B sees user A's message within ~5s without a refresh. Polling chosen over SSE/WebSockets because the HTTP endpoint is already the source of truth, Railway's free tier doesn't love long-lived connections, and the same pattern already powers the unread-count badge at a 30s cadence.
...
- [x] **Onboarding flow polished.** Search + center-filter UI over the 167 courses, selections persist to `enrollments` (renamed to `/my-classes` once enrollment-kind landed — see below).
...
- [x] **Enrollment kind: past / current / upcoming.** Migration `0005` drops `enrollments.is_current` and adds `kind` enum (CHECK in `('past','current','upcoming')`). Existing data backfilled: `is_current=true` → `current`, `is_current=false` → `past`. The page now lives at `/my-classes` (renamed from `/onboarding` — students come back every semester) with a per-course dropdown (Not enrolled / Current / Upcoming / Past), counts in the header, and an "Only show selected" filter. Matching reads from `kind IN ('current','upcoming')` for the buyer and `kind IN ('past','current')` for the seller — `upcoming` sellers don't get credited as having taken the class. Classmates lookup spans the viewer's full `(past, current, upcoming)` AND the other user's full `(past, current, upcoming)` — anyone with any course overlap shows up, with the OTHER user's kind returned per course for UI color-coding. New `DELETE /api/me/enrollments/{id}` so the UI can remove an enrollment when the user picks "Not enrolled." A persistent "Update my classes →" link in the For-my-courses header makes the page reachable any time.
- [x] **User settings page.** New `/settings` page lets users edit their `display_name` and upload an avatar. Email is read-only (sourced from Clerk; synthesized `@clerk.local` rows hidden in the UI). Backend routes `PATCH /api/me` and `POST /api/me/avatar`.
- [x] **Configure Clerk JWT template.** Done — session token now carries `name` / `email` / `picture` claims, and existing `@clerk.local` placeholder rows auto-migrate on next sign-in via the upsert's update branch. Surfaced a latent bug in the same pass: `ALLOWED_EMAIL_DOMAINS` defaulted to `"student.uaustin.org"` in config.py, so once Clerk started providing a real email, the previously-dormant domain check started rejecting non-UATX accounts in prod (local was fine because `.env` had it set empty, overriding the default). Fixed in PR #34 by flipping the code default to `""`, matching the documented "open to any Google account" policy.
- [x] **Bronze achieved.** Every Bronze-blocking item above is shipped; the Clerk JWT template step is a polish item (real emails/avatars instead of the `@clerk.local` fallback), not a functional gate.
...
- [x] **Bookmarkable URLs + back button** work end to end. Every page with internal state (tab, selected conversation, filter) carries that state in URL search params, so refresh and back/forward preserve where you were. Verified end-to-end across the IA restructuring.
- [x] **"Take down" hard-deletes listings** (PR #25). `DELETE /api/listings/{id}` removes the row (cascades to its conversations + messages via the schema-level CASCADE) and best-effort deletes the photo from Supabase Storage via `storage.delete_stored_image`. The ListingSettingsForm Take-down button confirms first, then calls the endpoint and navigates back to `/my-listings`. Replaces the old `status=withdrawn` behavior that left zombie rows + orphaned Storage objects.
- [x] **Visual design pass** with Tailwind: type scale, color palette, spacing, deliberate components. Done across multiple PRs: PR #28 (Sam — initial mobile + visual sweep), PR #40 (Eitan — UATX logo + favicon, amber-600 brand mark in the nav), PR #42 (Eitan — landing hero with logo + headline + three feature cards), PR #46 (Eitan — listing card hover lift + freshness signal + better empty states with "Clear filters" affordance), PR #48 (Eitan — mobile horizontal-scroll fix on listing cards via `min-w-0 flex-1`, plus `overflow-x-hidden` body safety net).
- [x] **Extra tests for silver behavior.** Went from ~9 backend tests at the end of Phase 0 to **161** across the silver + gold pushes. Notable test bundles: PR #36 added 10 cross-cutting authz + input-validation safety tests in `test_authz_safety.py` (outsider-can't-post-messages, 404 paths, Pydantic guards on price/payment-methods/empty-bodies). PR #45 added 4 feedback-submission tests. PR #46 added 2 search tests including LIKE-wildcard escaping. PR #33 added 17 mocked-Stripe tests covering 503-when-unconfigured, onboarding both branches, checkout 4xx cases, webhook signature verification and state transitions. We opted into "more tests" rather than the optional Playwright e2e — the spec frames it as an OR.

### Phase 3: Gold

- [x] **Mobile pass.** Done across PR #28 (Sam — initial mobile responsive layout, nav hamburger, two-pane chat collapse to list-or-thread below `md`, 44px tap targets) and PR #48 (Eitan — fixed a regression where listing cards on `/match`, `/everything-else`, and the "More from this seller" section were missing `min-w-0` on their flex text columns, causing long titles to push past the viewport on mobile; also added `overflow-x-hidden` on body as a defense-in-depth net).
...
- [x] **Custom feature 1: image uploads on listings.** Migration `0004` adds `listings.image_url`, `app/storage.py` proxies uploads to Supabase Storage, `POST /api/listings/{id}/image` enforces 5 MB cap + JPEG/PNG/WebP MIME + seller-only auth, NewListing form has a file picker, Browse + Detail render the image. Supabase bucket `listing-images` (public-read) and `SUPABASE_URL` / `SUPABASE_SERVICE_KEY` are configured on Railway; uploads verified working end-to-end. Avatars use the same bucket under `avatars/<user_id>/...`. Listing photos are removed on "Take down" via `storage.delete_stored_image` (best-effort).
- [x] **Custom feature 2: general-purpose marketplace ("Everything Else" tab).** Migration `0006` renames `book_title`→`title`, `book_author`→`author` (now nullable), `book_edition`→`edition`, and adds `category` (NOT NULL, CHECK in `book/furniture/electronics/clothing/kitchen/decor/sports/transportation/other`). Existing rows backfilled as `category='book'`. New endpoints: `GET /api/listings?category=non-book` filters to non-book items WITH a photo (image_url IS NOT NULL — items without one are hidden from the browse). New `/everything-else` page (image-heavy card grid with category filter + search) and `/everything-else/new` create form (photo required, no course tag). The existing `/listings` Browse is now books-only and dropped the inline thumbnails (books are recognizable from title + author + course; click into the listing to see the photo). My Listings gains a Books/Everything-else tab split via `?tab=`. The matching algorithm filters to `category='book'`. Top nav: My classes · For my courses · Classmates · My listings · My inquiries · Sell a book (with Everything Else reachable from the Books browse).
- [x] **(Bonus) Seller profile page.** Shipped. New `GET /api/users/{user_id}` returns a public profile (display name + avatar — deliberately excludes email and `stripe_onboarded`) plus the seller's active listings. New `/users/:userId` page renders the profile, listings grid, and a "Message me" button that creates (or reopens) a DM via the existing `POST /api/users/{id}/dm` endpoint. Reachable by clicking the seller's name on any listing detail. Self-view replaces the "Message me" button with a "Manage your listings" link.
- [x] **README updated** with gold-tier description: both nontrivial pieces (matching + classmates), both custom features (image uploads + general marketplace), and the pick-one (real-time chat). See [README.md](README.md) → Nontrivial logic.
