...
- [~] **UX restructuring: replace Inbox with per-context chat homes** (per Sam's ask, multi-PR series). **Backend foundation [x]** in PR #17. **`/my-listings` page [x]** in PR #18: index lists my listings with thumbnail + unread pill per card; detail page has Chat + Settings subtabs (state in `?tab=&conv=` query params so URLs are bookmarkable). Chat subtab uses the seller-only `GET /api/listings/{id}/conversations` and renders the per-buyer thread inline via the new reusable `<ConversationThread>` component. Settings subtab edits all listing fields (title/author/edition/condition/price/course/description), uploads a replacement photo, and changes status — incl. "Take down" = withdrawn. `ListingUpdate` schema extended to accept all those fields. Nav gets a new "My listings" link with its own per-context unread badge from `counts.listings`. **Still to build:** The current `Inbox` page mixes every kind of conversation a user has. Restructure into three contexts, each owning its own chats and notification badge:
  - **`/my-listings`** — the seller's home for listings they posted. Shows each listing as a card with image / title / status / unread-message count. Click a listing → tabs:
    - **Chat** sub-tab: list of buyers who messaged about this listing (each row a buyer + their last message + unread count); click a buyer → see the thread inline.
    - **Settings** sub-tab: edit the listing fields (price, description, condition), upload/replace photo, change status (active → reserved → sold → withdrawn). "Take down" = set status='withdrawn'.
  - **`/my-inquiries`** (buying-side page — name locked in per Sam). Lists listing-scoped conversations where I'm the buyer (`conversation.buyer_id = me AND listing_id IS NOT NULL`). Each row: listing title + thumbnail + seller + unread count. Click → enter the thread.
  - **Classmates** becomes the home for DMs (the click-to-DM feature shipped in PR #12 already routes through `/inbox/<id>` — that target page moves into the Classmates UX). Each classmate row gets an unread badge for the DM with them. Click → DM thread inline. Removes the need for a separate Inbox.
  - **Remove the `/inbox` top-level page entirely** once all three contexts have absorbed their chats. The conversation-detail route can stay (`/inbox/:id`) so existing links don't break, or get moved under `/my-listings/:id/chat/:buyer` and `/buying/:listing_id` and `/classmates/:user_id`.
  - **Nav badge restructure**: instead of one Inbox badge, three smaller badges (My Listings, Buying, Classmates). Backend: `GET /api/me/unread-counts` returning `{listings: N, buying: N, dms: N}`. Frontend: `UnreadContext` exposes the breakdown.
- [ ] **Settings page polish** (per Sam's ask):
  - **Profile picture upload** — same Supabase Storage pattern as listing images, but stored at `avatars/<user_id>/...`. New `POST /api/me/avatar` endpoint; settings page gets a file picker; `users.avatar_url` updated. Existing field already exists from Clerk-claim sync.
  - **Hide the `@clerk.local` placeholder email** in the UI. Either: don't render the Email row when `user.email` ends in `@clerk.local`, OR replace it with a "Configure your real email in Clerk" hint. Real emails (once the Clerk JWT template is configured) still show.
