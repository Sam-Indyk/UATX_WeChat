...
- [~] **UX restructuring: replace Inbox with per-context chat homes** (per Sam's ask, multi-PR series). **Backend foundation [x]** in PR #17: `GET /api/me/unread-counts` returns the per-context breakdown `{listings, inquiries, dms, total}`; `GET /api/me/listings` returns the seller's listings with per-row `unread_count`; `GET /api/listings/{id}/conversations` is the seller-only view of all buyers on a listing with per-thread `unread_count`. `UnreadContext` on the frontend now exposes `counts: UnreadCounts` instead of a single number. **Still to build:** The current `Inbox` page mixes every kind of conversation a user has. Restructure into three contexts, each owning its own chats and notification badge:
