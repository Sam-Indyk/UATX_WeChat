...
- [~] **UX restructuring: replace Inbox with per-context chat homes** (per Sam's ask, multi-PR series). **Backend foundation [x]** in PR #17. **`/my-listings` page [x]** in PR #18. **`/my-inquiries` page [x]** in PR #19. **Classmates-as-DM-home [x]** in PR #20: `/api/classmates` now returns `dm_conversation_id` and `unread_count` per row; the page becomes a two-pane (classmate list + inline DM thread, `?dm=<id>` URL param). Click a classmate → creates the DM if needed, then opens it inline (no more nav to `/inbox/:id`). Classmates nav link gets a `counts.dms` badge. **Still to build:** The current `Inbox` page mixes every kind of conversation a user has. Restructure into three contexts, each owning its own chats and notification badge:
