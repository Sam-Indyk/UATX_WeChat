...
- [ ] **Smoke-test bronze end-to-end on the live URL.** Open the deployed app in a fresh browser, sign in, walk through every page, document every bug. Fix the top ones in follow-up PRs. CI can't catch UI-level breakage — this is the only way to find it.
