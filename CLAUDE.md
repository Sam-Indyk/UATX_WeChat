...
- [ ] **Enrollment kind: past / current / upcoming.** Replace `enrollments.is_current` (boolean) with `kind` enum (`past`, `current`, `upcoming`). Migrate existing data: rows with `is_current=true` become `current`, rows with `is_current=false` become `past`. Update Onboarding UI to let users mark each course as one of the three states (default current). Update matching to surface books for the user's `current` AND `upcoming` courses. Update classmates to optionally include upcoming-shared classes. Add tests for the new states.
...
- [ ] **UX restructuring: replace Inbox with per-context chat homes** (per Sam's ask, big chunk — should be its own PR series). The current `Inbox` page mixes every kind of conversation a user has. Restructure into three contexts, each owning its own chats and notification badge:
