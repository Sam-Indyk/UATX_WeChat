...
- [~] **UX restructuring: replace Inbox with per-context chat homes** (per Sam's ask, multi-PR series). **Backend foundation [x]** in PR #17. **`/my-listings` page [x]** in PR #18. **`/my-inquiries` page [x]** in PR #19: new `GET /api/me/inquiries` returns listing-scoped conversations where I'm the buyer (with per-row `unread_count`), and the page renders the same two-pane convo-list + inline-thread layout as the My Listings chat subtab. Also fixed a latent bug where `conversations.updated_at` never actually bumped on new messages (the old code assigned `msg.created_at` which is None pre-commit) — Inbox sort and inquiry sort both depended on this. **Still to build:** The current `Inbox` page mixes every kind of conversation a user has. Restructure into three contexts, each owning its own chats and notification badge:
