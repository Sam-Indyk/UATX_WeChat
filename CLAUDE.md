...
- [x] **Custom feature 2: general-purpose marketplace ("Everything Else" tab).** Migration `0006` renames `book_title`→`title`, `book_author`→`author` (now nullable), `book_edition`→`edition`, and adds `category` (NOT NULL, CHECK in `book/furniture/electronics/clothing/kitchen/decor/sports/transportation/other`). Existing rows backfilled as `category='book'`. New endpoints: `GET /api/listings?category=non-book` filters to non-book items WITH a photo (image_url IS NOT NULL — items without one are hidden from the browse). New `/everything-else` page (image-heavy card grid with category filter + search) and `/everything-else/new` create form (photo required, no course tag). The existing `/listings` Browse is now books-only and dropped the inline thumbnails (books are recognizable from title + author + course; click into the listing to see the photo). My Listings gains a Books/Everything-else tab split via `?tab=`. The matching algorithm filters to `category='book'`. Top nav now: Books · Everything else · For my courses · Classmates · My listings · My inquiries · Sell a book.
